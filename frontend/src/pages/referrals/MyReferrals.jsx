    ...
    <div className="flex-1 min-w-0 bg-[#f8fafc] pb-12 min-h-screen">
      ...
      <div className="flex flex-wrap gap-2 p-1.5 bg-slate-200/60 rounded-xl w-fit">
        {[
          ...
          { key: 'pending', label: `Pending (${items.filter((r) => r.status === 'pending').length})` },
          { key: 'shortlisted', label: `Shortlisted (${items.filter((r) => r.status === 'shortlisted').length})` },
          { key: 'selected', label: `Selected (${items.filter((r) => r.status === 'selected').length})` },
          { key: 'rejected', label: `Rejected (${items.filter((r) => r.status === 'rejected').length})` }
        ].map((tab) => (
          <button
            key={tab.key}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {filteredItems.map((r) => {
            const dateStr = r.date ? new Date(r.date).toLocaleDateString('en-GB') : 'Recent';
                      ...
                      <p className="text-xs text-slate-500 flex items-center gap-2">
                        <span className="flex items-center gap-1">
                          ...
                          {isStudent
                            ...
                            : `Candidate: ${r.studentId?.name || 'Student'}`}
                        </span>
                        <span>•</span>
                        <span className="flex items-center gap-1">
              ...
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-100 space-y-1">
                ...
                {isStudent ? (
                  <Select
                    ...
                    disabled={updatingId === selectedReferral._id}
                    onChange={(e) => handleStatusUpdate(selectedReferral._id, e.target.value)}
