...
export function Dialog({ open, onOpenChange, children, className = '' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Dialog Box */}
      <div
        className={`relative z-50 w-full max-w-lg rounded-2xl border border-slate-700 bg-slate-900 p-6 text-slate-100 shadow-2xl max-h-[90vh] overflow-y-auto ${className}`}
...
export function DialogFooter({ className = '', children, ...props }) {
  return (
    <div className={`flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-3 mt-6 pt-4 border-t border-slate-800 ${className}`} {...props}>
      {children}
    </div>
  );
}
