      ...
      async function loadMentors() {
        try {
          ...
          if (Array.isArray(list) && list.length > 0 && !selectedAlumniId) {
            setSelectedAlumniId(list[0].userId?._id || list[0]._id);
          }
        } catch (err) {
            ...
            {!initialAlumniId && (
              <div className="space-y-1.5">
                ...
                {alumniList.length === 0 ? (
                  ...
                  >
                    {alumniList.map((al) => {
                      const uId = al.userId?._id || al._id;
                      const name = al.userId?.name || al.name || 'Alumni Mentor';
                      const role = al.jobRole ? `${al.jobRole} @ ${al.company || 'Tech'}` : 'Alumni';
                      return (
