...
export default function Sidebar({ activeTab, setActiveTab, collapsed, setCollapsed }) {
  ...
  return (
    <aside
      className={`fixed left-0 top-0 bottom-0 z-30 bg-slate-950/80 backdrop-blur-2xl border-r border-white/5 transition-all duration-300 flex flex-col shadow-2xl shadow-black/50 ${
        collapsed ? 'w-20' : 'w-72'
      }`}
    >
      {/* Brand Header */}
      <div className="h-[76px] flex items-center justify-between px-5 border-b border-white/5">
        <div 
      ...
      <nav className="flex-1 py-6 px-4 space-y-6 overflow-y-auto custom-scrollbar">
        {navSections.map((section, sIdx) => (
          <div key={sIdx}>
            <div className={`px-2 mb-3 text-[11px] font-bold uppercase tracking-widest text-slate-500 ${collapsed ? 'sr-only' : 'block'}`}>
              {section.title}
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3.5">
                      <Icon className={`w-5 h-5 shrink-0 transition-colors duration-200 ${isActive ? 'text-white' : 'text-slate-500 group-hover:text-slate-300'}`} />
      ...
      {!collapsed ? (
        <div className="p-4 m-4 bg-slate-900/50 backdrop-blur-md border border-white/10 rounded-2xl space-y-3 relative overflow-hidden group">
          {/* Subtle glowing effect behind card */}
          ...
          <div className="flex items-center justify-between text-xs font-semibold relative z-10">
            <span className="text-[10px] uppercase tracking-widest text-slate-500">Current Role:</span>
          ...
          <div className="grid grid-cols-3 gap-1 bg-black/20 p-1 rounded-xl border border-white/5 text-[11px] relative z-10">
            <button
              onClick={() => {
                switchRole('admin');
              ...
              className={`py-1.5 rounded-lg font-medium transition-all ${
                role === 'admin' ? 'bg-white/10 text-white font-semibold shadow-sm' : 'text-slate-500 hover:text-slate-300'
              }`}
            >
              Admin
            </button>
            <button
              ...
              className={`py-1.5 rounded-lg font-medium transition-all ${
                role === 'alumni' ? 'bg-white/10 text-white font-semibold shadow-sm' : 'text-slate-500 hover:text-slate-300'
              }`}
            >
              Alumni
            </button>
            <button
              ...
              className={`py-1.5 rounded-lg font-medium transition-all ${
                role === 'student' ? 'bg-white/10 text-white font-semibold shadow-sm' : 'text-slate-500 hover:text-slate-300'
              }`}
            >
              Student
            </button>
          ...
          <div className="pt-2 flex items-center gap-3 relative z-10">
            <img
              ...
              alt={user?.name}
              className="w-8 h-8 rounded-full object-cover border-2 border-slate-800 shadow-sm"
            />
            <div className="min-w-0 flex-1">
              <p className="text-xs font-semibold text-slate-200 truncate leading-tight">{user?.name}</p>
              <p className="text-[10px] text-slate-500 truncate mt-0.5">{user?.company || user?.department}</p>
            </div>
            <button
              onClick={logout}
              title="Sign out"
              className="p-2 rounded-xl text-slate-500 hover:text-red-400 hover:bg-red-500/10 transition-colors shrink-0 cursor-pointer"
            >
        ...
        <div className="p-4 flex flex-col items-center gap-3 border-t border-white/5 bg-slate-900/50">
          <img
            src={user?.avatar || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150'}
            alt={user?.name}
          ...
          <button
            onClick={logout}
            title="Sign out"
            className="p-2.5 rounded-xl text-slate-500 hover:text-red-400 hover:bg-red-500/10 transition-colors cursor-pointer"
          >
            <LogOut className="w-4 h-4" />
          </button>
        </div>
