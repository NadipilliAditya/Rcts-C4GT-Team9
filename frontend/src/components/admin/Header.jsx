...
export default function Header({ 
  ...
  const dismissNotif = (e, id) => {
    ...
    setNotifications(prev => ({
      ...prev,
      [activeRole]: (prev[activeRole] || []).filter(n => n.id !== id)
    }));
  };
  ...
  return (
    <header className="h-16 bg-white border-b border-slate-200 sticky top-0 z-20 px-4 sm:px-6 flex items-center justify-between shadow-xs gap-3">
      <div className="flex items-center gap-3 min-w-0">
        {/* 3-Line Hamburger Menu Toggle Button - Always present on all tabs */}
        <button
          onClick={() => {
              ...
              <div className="pt-2 flex items-center justify-end gap-3">
                ...
                {(() => {
                  ...
                  return (
                    <button
                      onClick={() => {
                        alert(`Action triggered: "${selectedNotif.actionText}"`);
                        setSelectedNotif(null);
                      }}
                    ...
                    >
                      {selectedNotif.actionText || 'Take Action'} →
                    </button>
                  );
                })()}
              </div>
